...
function voxelTerrain(opts) {
  ...
  const villageRoadNoise = indevGenerator.simplex({
    min: 0,
    max: 1,
    frequency: 1,
    octaves: 2,
  });
  const villageRoadMaxLengthNoise = indevGenerator.simplex({
    min: 0,
    max: 1,
    frequency: 1,
    octaves: 2,
  });
  const villageRoadLengthNoise = indevGenerator.simplex({
    min: 0,
    max: 1,
    frequency: 1,
    octaves: 2,
  });
  const villageRoadDirectionNoise = indevGenerator.uniform({
    min: 0,
    max: 1,
    frequency: 1,
    octaves: 1,
  ...
  const villageBuildingNoise = indevGenerator.simplex({
    min: 0,
    max: 1,
    frequency: 1,
    octaves: 1,
  });
  const villageBuildingTypeNoise = indevGenerator.uniform({
    min: 0,
    max: 1,
    frequency: 1,
    octaves: 1,
  ...
  return function generateChunk(position) {
    ...
    const metadata = new Uint8Array(chunkSize * chunkSize * chunkSize * BLOCK_METADATA_BYTES);
    const vegetations = {};
    const weathers = [];
    const effects = {};
    const entities = {};
    ...
    const getHeight = (() => {
      ...
      return (x, z) => {
        ...
        const idx = getIndexPadded(rawX, rawZ, paddedChunkSize);
        let height;
    ...
    function getIndexPadded(x, z, paddedChunkSize) {
      return (x) + (z * paddedChunkSize);
    }
    ...
    function generatePointsChunkPadded() {
      forEachChunk((chunkX, chunkZ) => {
        ...
        VILLAGES.make({
          ...
          roadNoise: villageRoadNoise,
          roadMaxLengthNoise: villageRoadMaxLengthNoise,
          roadLengthNoise: villageRoadLengthNoise,
          roadDirectionNoise: villageRoadDirectionNoise,
          buildingNoise: villageBuildingNoise,
          buildingTypeNoise: villageBuildingTypeNoise,
          setVoxel: setVoxelInside,
        });
      ...
      for (let y = startY; y <= h; y++) {
        ...
        if (material !== null) {
          if (material === WATER_VALUE) {
            setVoxel(x, y, z, WATER_VALUE, {depth: FULL_VALUE})
    ...
    function postRivers() {
      ...
      for (let i = 0; i < riverSurfaces.length; i++) {
        ...
        for (let y = h; y <= riverSurfacesMedianHeight; y++) {
          setVoxel(x, y, z, WATER_VALUE, {depth});
        }
      }
    }
    ...
    function setVoxel(x, y, z, value, metadata) {
      const idx = vu.getIndex(x, y, z);
      voxels[idx] = value;
      if (metadata) {
        vu.setBlockMetadata(metadata.buffer, idx, metadata);
      }
    }
    ...
    function setVoxelInside(x, y, z, value, metadata) {
      if (isInside(x, y, z)) {
        const idx = vu.getIndex(x, y, z);
        voxels[idx] = value;
        if (metadata) {
