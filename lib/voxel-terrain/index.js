...
function voxelTerrain(opts) {
  ...
  return function generateChunk(position) {
    ...
    const getUsed = (() => {
      let usedMap = null;
      let usedMapIndex = null;

      return (x, z) => {
        if (usedMap === null) {
          usedMap = new Uint8Array(paddedChunkSize * paddedChunkSize);
          usedMapIndex = new Uint8Array(paddedChunkSize * paddedChunkSize);
        }
