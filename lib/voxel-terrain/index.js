...
function voxelTerrain(opts) {
  ...
  return function generateChunk(position) {
    ...
    let _heightMap = null;
    function _getHeightMap() { // XXX make this map extend out past to padding
      if (_heightMap === null) {
        _heightMap = new Uint16Array(chunkSize * chunkSize);
        forEachPoint((x, z) => {
          const height = floor(terrainNoise.in2D(x, z));
          const idx = vu.getIndex2D(x, z);
          _heightMap[idx] = height;
        });
      }
      return _heightMap;
    }
    let _usedMap = null;
    function _getUsedMap() {
      if (_usedMap === null) {
        _usedMap = new Uint8Array(chunkSize * chunkSize);

        const heightMap = _getHeightMap();
        forEachPoint((x, z) => {
          const idx = vu.getIndex2D(x, z);
    ...
    function isLandSurface(x, z) {
      const usedMap = _getUsedMap();
      const idx = vu.getIndex2D(x, z);
      return usedMap[idx] === 0;
    }
