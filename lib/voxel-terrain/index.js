...
function voxelTerrain(opts) {
  ...
  return function generateChunk(position) {
    ...
    // XXX demand-compute and make sure they apply to the surface chunk and not the current one
    const heightMap = new Float64Array(chunkSize * chunkSize);
    const usedMap = new Uint8Array(chunkSize * chunkSize);
    const riverSurfaces = [];
