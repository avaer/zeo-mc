...
function Atlas(x, y, w, h, tilepad) {
  ...
  if (arguments.length === 2) {
    w = x; h = y; x = y = 0; tilepad = false;
  }
  this.left = this.right = null;
  this.rect = new Rect(x, y, w, h, tilepad);
  this.filled = false;
  this._cache = [];
...
module.exports = function() {
  ...
  return new Atlas(arguments[0], arguments[1], arguments[2], arguments[3], arguments[4]);
};
...
Atlas.prototype.pack = function(rect) {
  ...
  if ((this.rect.w - rect.w) > (this.rect.h - rect.h)) {
    this.left = new Atlas(this.rect.x, this.rect.y, rect.w, this.rect.h, this.rect.tilepad);
    this.right = new Atlas(this.rect.x + rect.w, this.rect.y, this.rect.w - rect.w, this.rect.h, this.rect.tilepad);
  } else {
    this.left = new Atlas(this.rect.x, this.rect.y, this.rect.w, rect.h, this.rect.tilepad);
    this.right = new Atlas(this.rect.x, this.rect.y + rect.h, this.rect.w, this.rect.h - rect.h, this.rect.tilepad);
  }
...
Atlas.prototype.expand = function(rect) {
  ...
  if (this.rect.w < this.rect.h) {
    atlas = new Atlas(0, 0, this.rect.w + rect.w, this.rect.h, this.rect.tilepad);
    atlas.right = new Atlas(this.rect.w, 0, rect.w, this.rect.h, this.rect.tilepad);
    atlas.left = this;
  } else {
    atlas = new Atlas(0, 0, this.rect.w, this.rect.h + rect.h, this.rect.tilepad);
    atlas.right = new Atlas(0, this.rect.h, this.rect.w, rect.h, this.rect.tilepad);
    atlas.left = this;
  }
...
Atlas.prototype.uv = function(w, h) {
  ...
  (function loop(atlas) {
    if (atlas.left !== null) {
      ...
      var p = atlas.rect.tilepad ? atlas.rect.w / 4 : 0;
...
Atlas.prototype.json = function(input) {
  ...
  if (input) {
    ...
    return (function loop(obj) {
      ...
      var atlas = new Atlas(obj.rect.x, obj.rect.y, obj.rect.w, obj.rect.h, obj.rect.tilepad);
      if (obj.rect.name) atlas.rect.name = obj.rect.name;
...
Atlas.prototype._tilepad = function(rect) {
  ...
  canvas.id = img.id || '';
  canvas.tilepad = img.tilepad || false;
  ...
  return new Rect(rect.x, rect.y, this.img.width, this.img.height, true);
};
...
Atlas.prototype._ontoCanvas = function(node) {
  if (node && this.img && this.canvas) {
    ...
    node.rect.tilepad = this.img.tilepad || false;
  }
  return node;
};
...
Atlas.prototype._toRect = function(rect) {
  ...
  if (rect.nodeName && rect.nodeName === 'IMG') {
    this.img = rect;
    rect = new Rect(rect.x, rect.y, rect.width, rect.height, rect.tilepad);
  }
  // if rect is an object
  if (!(rect instanceof Rect)) {
    rect = new Rect(rect.x || 0, rect.y || 0, rect.w || rect.width, rect.h || rect.height, rect.tilepad);
  }
  return rect;
};
