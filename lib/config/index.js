...
const u = require('../utils/js-utils/index.js');

const DIFFIE_HELLMAN_BITS = 2048;
...
const api = {
  bootstrap(cb) {
    this._getInitialization(u.ok(cb, initialization => {
      if (initialization) {
        _latchInitialization(initialization);

        cb();
      } else {
        this._setInitialization(u.ok(cb, initialization => {
          _latchInitialization(initialization);
  ...
  _setInitialization(cb) {
    const dh = crypto.createDiffieHellman(DIFFIE_HELLMAN_BITS);
    ...
    mkdirp(keysDirectory, u.ok(cb, () => {
      u.parallel(() => {
        ...
        fs.writeFile(publicKeyPath, 'utf8', cb);
      }, () => {
