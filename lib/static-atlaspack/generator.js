...
function staticAtlaspackGenerator(cb) {
  function loadTextures(atlas, cb) {
    ...
    const itemMaterials = SPRITES.ITEMS;
    ...
    const getBlockImg = (material, cb) => {
      getImgFile(path.join(__dirname, '..', '..', 'assets', 'img', 'blocks', material + '.png'), (err, img) => {
        if (!err) {
    ...
    const getModelImg = (material, cb) => {
      getImgFile(path.join(__dirname, '..', '..', 'assets', 'img', 'entity', material + '.png'), (err, img) => {
        if (!err) {
          img.id = material;
          img.tilepad = false;
          cb(null, img);
    ...
    const getItemImg = (item, cb) => {
      getImgFile(path.join(__dirname, '..', '..', 'assets', 'img', 'items', item + '.png'), (err, img) => {
        if (!err) {
          img.id = item;
          img.tilepad = false;
          cb(null, img);
    ...
    serial([
      ...
      loadMaterials(itemMaterials, getItemImg),
    ], cb);
  }
